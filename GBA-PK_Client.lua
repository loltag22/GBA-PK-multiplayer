...
if not (emu == nil) then Globals.ROMCARD = emu.memory.cart0 end
...
--If 0 then don't render players
	...
	for i = 1, MaxPlayers do
		 MVars.PlayerVis[i] = 0
		 MVars.MultiplayerConsoleFlags[i] = 0
	...
	elseif BattleVars[1] == 1 and BattleVars[11] < 150 then
		BattleVars[11] = BattleVars[11] + 1
			...
			if BattleVars[11] >= 150 then
				--Set enemy team
				PokemonTeamManager.SetEnemyPokemonTeam(0,1, Globals.GameID)
				BattleVars[1] = 2
			end
	...
	elseif BattleVars[1] == 2 then
		...
		elseif BattleVars[6] == 1 then
			...
			--	BattleVars[13] = ReadBuffers()
				Globals.ConsoleForText:advance(1)
	...
	elseif TradeVars[1] == 3 then
		...
		elseif Globals.Var8000[2] == 2 and OtherPlayerHasCancelled == 0 then
			...
			if EnemyTradeVars[1] == 4 and EnemyTradeVars[2] == 2 then
				...
				PokemonTeamManager.SetEnemyPokemonTeam(TeamPos, 1, Globals.GameID)
...
function CalculateCamera()
	...
	--	Globals.ConsoleForText:print("PlayerMapXMove: " .. PlayerMapXMove .. "PlayerMapYMove: " .. PlayerMapYMove .. "PlayerMapXMovePREV: " .. PlayerMapXMovePrev .. "PlayerMapYMovePrev: " .. PlayerMapYMovePrev)
...
function GetNewGame()
	...
	if Globals.ConsoleForText == nil then
		Globals.ConsoleForText = console:createBuffer("GBA-PK CLIENT")
	end
...
function CreateNetwork()
	if MasterClient ~= "c" then
		...
		Globals.ConsoleForText:print("Searching for an open game on IP ".. IPAddress .. " and port " .. Port)
		Globals.ConsoleForText:moveCursor(0,15)
		Globals.ConsoleForText:print("Connected to a server: No                   ")
		ReceiveData()
	else
		if PlayerID ~= 1 then
			Globals.ConsoleForText:moveCursor(0,3)
			Globals.ConsoleForText:print("You have successfully connected.                                                          ")
			Globals.ConsoleForText:moveCursor(0,15)
			Globals.ConsoleForText:print("Connected to a server: Yes                 ")
		else
			Globals.ConsoleForText:moveCursor(0,3)
			Globals.ConsoleForText:print("Searching for an open game on IP ".. IPAddress .. " and port " .. Port)
			Globals.ConsoleForText:moveCursor(0,15)
...
function ReceiveData()
	...
	if Globals.EnableScript then
			...
			if SocketMain:hasdata() then
				...
				if ReadData ~= nil then
					...
					MVars.PlayerReceiveID = ReceiveDataSmall[4]
	...
	--					Globals.ConsoleForText:print("Valid package! Contents: " .. ReadData)
				...
				--			Globals.ConsoleForText:print("Other player has canceled trade.")
					...
					--		Globals.ConsoleForText:print("Other player is too busy to trade.")
						...
						if ReceiveDataSmall[5] == "STRT" then
							...
							MVars.PlayerIDNick[1] = ReceiveDataSmall[2]
							PlayerID2 = tonumber(ReceiveDataSmall[6])
							PlayerID = PlayerID2 - 1000
							MVars.FutureX[1] = ReceiveDataSmall[7]
							MVars.FutureY[1] = ReceiveDataSmall[8]
						--	Facing = tonumber(ReceiveDataSmall[11])
							--Action type -> can be walk or run
							MVars.PlayerExtra1[1] = ReceiveDataSmall[10]
							MVars.PlayerExtra2[1] = ReceiveDataSmall[11]
							MVars.PlayerExtra3[1] = ReceiveDataSmall[12]
							MVars.PlayerExtra4[1] = ReceiveDataSmall[13]
						end
		...
		--			if TempVar2 == 0 then Globals.ConsoleForText:print("Test 4") end
			--		else
			--			Globals.ConsoleForText:print("INVALID PACKAGE: " .. ReadData)
					end
				end
			end
	end
...
function CreatePackettSpecial(RequestTemp, OptionalData)
	if RequestTemp == "POKE" then
		...
		PokemonTeamManager.GetPokemonTeam(Globals.GameID)
		local PokeTemp
		local StartNum = 0
		local StartNum2 = 0
		...
		--	console:log("PACKETT SENDING TO " .. MVars.PlayerReceiveID .. ": " .. Packett)
			end
		end
...
function ConnectNetwork()
	...
	--Receive timer
		...
		if SendTimer == 0 then 
			...
			for i = 1, MaxPlayers do
				MVars.PlayerReceiveID = i + 1000
				if PlayerID2 ~= MVars.PlayerReceiveID then
					SendData("SPOS")
				end
			end
			MVars.timeout[1] = MVars.timeout[1] - 4
					if MVars.timeout[1] <= 0 then
						...
						Globals.ConsoleForText:print("You have been disconnected due to timeout.                                           ")
						Globals.ConsoleForText:moveCursor(0,15)
						Globals.ConsoleForText:print("Connected to a server: No         ")
						for i = 1, MaxPlayers do
		...
		ReceiveData()
	--	Globals.ConsoleForText:print("Connection Type: " .. ReturnConnectionType)
		if ReturnConnectionType == "STRT" then
			console:log("You have connected to " .. MVars.PlayerIDNick[1])
			Globals.ConsoleForText:moveCursor(0,3)
			Globals.ConsoleForText:print("You have successfully connected.                                                          ")
			Globals.ConsoleForText:moveCursor(0,15)
			Globals.ConsoleForText:print("Connected to a server: Yes                 ")
...
function Interact()
	if Globals.EnableScript and initialized ~= 0 then
...
function mainLoop()
	...
	if initialized == 0 and Globals.EnableScript then
	...
	--	ScriptLoader.Loadscript(0)
		...
		Globals.ConsoleForText:moveCursor(0,3)
		CreateNetwork()
	elseif Globals.EnableScript then
			--Debugging
			...
			if TempVarTimer == 0 then
				if MVars.timeout[1] == 0 then
					CreateNetwork()
							...
							--VARS--
		if Globals.GameID == "BPR1" or Globals.GameID == "BPR2" then
			--this is the ram location where the scripts are stored
...
--				if SendTimer == 0 then SendData("RTRA") end
		...
		--	if Globals.Var8000[2] ~= 0 then Globals.ConsoleForText:print("Var8001: " .. Globals.Var8000[2]) end
				...
				elseif Globals.Var8000[2] == 1 then LockFromScript = 0 SendData("DBAT") Keypressholding = 1 end
		...
		--	if Globals.Var8000[2] ~= 0 then Globals.ConsoleForText:print("Var8001: " .. Globals.Var8000[2]) end
				...
				elseif Globals.Var8000[2] == 1 then LockFromScript = 0 SendData("DTRA") Keypressholding = 1 end
			end
	end
end
...
if not (emu == nil) then
	if Globals.ConsoleForText == nil then Globals.ConsoleForText = console:createBuffer("GBA-PK CLIENT") end
	Globals.ConsoleForText:clear()
	Globals.ConsoleForText:moveCursor(0,1)
	FFTimer2 = os.time()
	GameChecker.GetGameVersion()
